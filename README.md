...
+ t_permission（权限表）
  ...
  + name
    ...
    具体权限的中文名字，比如：`用户管理`，`用户管理-列表`，`用户管理-``用户管理-新增`，`用户管理-删除`，`交易管理`，`交易管理-新增`，`交易管理-更新`等等
...
我们的流程是：用户登录 -> 给用户配置角色 -> 给角色配置能访问的资源。在前面六章的代码中，我们早就完成了用户登录，于是我们开始下一步：给用户配置角色。大致的想法就是：给我们的用户实体类添加一个权限的属性，再在用户的 Service 类里 loadUserByUsername 方法中，返回添加了权限属性的用户对象，下面来看具体实现：（注意先把 t_role 这个权限表的 MyBatis 三兄弟的代码自动生成好）
...
不加注解的，都可以访问，加了注解的，要有对应权限才可以访问。举个例子：
...
其实很简单，若你数据库中 t_role 表的 role 字段的值为类似 `admin` 的，用 hasAnyAuthority。 role 字段的值为类似 `ROLE_admin` 的，用 hasRole。使用 hasRole 方法会判断你得权限的值是带前缀 `ROLE_` 的，正常写的值得话，你就用 hasAnyAuthority 方法。

至此，SpringSecurity 框架的权限管理就完成了。

另外，我在提一个东西，就是，现在如果我们访问没有权限的页面，浏览器会自动跳转到默认的 403 页面，这个页面不是很好看，像是报错一样，我们现在可以使用自己定义一个 403 页面。
...
以上，就是我们的 SpringSecurity 框架的权限管理。当然了，我之前说过，SpringSecurity 框架权限管理基于两种，一种是基于角色的权限管理，还有一种是基于资源的权限管理。上面这种就是基于角色的权限管理。现在我们来讲另一种，基于资源的权限管理。
